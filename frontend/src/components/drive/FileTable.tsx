  ...
  return (
    <div className="mt-5">
      ...
      <table className="w-full min-w-[760px] border-collapse text-left text-sm">
        ...
        <tbody>
          {files.map((file) => (
            <tr key={file.id ?? file.name} onContextMenu={(event) => onFileContextMenu?.(event, file)} onClick={() => onToggleFile?.(file)} onDoubleClick={() => onFileDoubleClick?.(file)} className={selectedFileIds.has(file.id ?? '') ? 'border-b border-blue-100 bg-blue-50 transition hover:bg-blue-50' : 'border-b border-slate-200 transition hover:bg-slate-50'}>
