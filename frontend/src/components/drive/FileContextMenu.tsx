import { Download, Edit3, Eye, FolderInput, Info, Link2, Trash2, UserPlus, FileEdit } from 'lucide-react'
import { Button } from '@/components/ui/button'
import type { FileItem } from '@/data/drive-data'

export function FileContextMenu({ x, y, file, onClose, onView, onDownload, onRename, onMove, onDetails, onShare, onInvite, onDelete, onEditOnlyOffice }: { x: number; y: number; file: FileItem | null; onClose: () => void; onView: () => void; onDownload: () => void; onRename: () => void; onMove: () => void; onDetails: () => void; onShare: () => void; onInvite: () => void; onDelete: () => void; onEditOnlyOffice?: () => void }) {
  if (!file) return null
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-3 bottom-3 z-50 max-h-[calc(100vh-24px)] overflow-y-auto rounded-3xl border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-900/15 sm:inset-x-auto sm:bottom-auto sm:w-52 sm:rounded-2xl" style={{ left: window.innerWidth >= 640 ? safeX : undefined, top: window.innerWidth >= 640 ? safeY : undefined }}>
        <p className="truncate px-3 py-2 text-xs font-bold text-slate-500">{file.name}</p>
        {['xlsx', 'docx', 'pptx', 'xls', 'doc', 'ppt', 'csv'].some(ext => (file.name || '').toLowerCase().trim().endsWith('.' + ext)) ? null : (
          <Button variant="ghost" className="h-12 w-full justify-start sm:h-11" onClick={onView}><Eye className="h-4 w-4" />View</Button>
        )}
        {onEditOnlyOffice && ['xlsx', 'docx', 'pptx', 'xls', 'doc', 'ppt', 'csv'].some(ext => (file.name || '').toLowerCase().trim().endsWith('.' + ext)) ? (
          <Button variant="ghost" className="h-12 w-full justify-start sm:h-11" onClick={onEditOnlyOffice}><FileEdit className="h-4 w-4 text-blue-600" /><span className="text-blue-600">Open/edit</span></Button>
        ) : null}
